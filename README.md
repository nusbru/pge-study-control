...
Crie o ambiente com segredos URL-safe, valide-o e suba os tres servicos:
...
O validador interrompe a sequencia antes do build se algum `CHANGE_ME` permanecer. A URL externa do health check e `http://127.0.0.1:<APP_PORT>/api/health`, com `<APP_PORT>` igual ao valor configurado em `.env`; o comando acima verifica a mesma rota por dentro do container, sem depender da porta do host.
